...
class ScrollingLabel extends St.ScrollView {
    ...
    createScrollAnimation(adjustment, origText) {
        ...
        this.transition.connect("completed", () => {
            ...
            if (this.scrollPauseTime > 0) {
                ...
                this.cyclePauseTimerId = GLib.timeout_add(GLib.PRIORITY_DEFAULT, this.scrollPauseTime, () => {
                    this.cyclePauseTimerId = null;
                    if (!this.initPaused) {
                        this.transition.start();
                    }
     ...
     */
    processLabelWidth() {
        const [minW, naturalW] = this.label.get_preferred_width(-1);
        const actualWidth = Math.max(naturalW, this.label.width);
        const isLabelWider = actualWidth > this.labelWidth && this.labelWidth > 0;
